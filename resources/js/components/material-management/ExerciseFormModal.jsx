export default function ExerciseFormModal({ open, onClose, onSave, activeChapter }) {
    if (!open) {
        return null;
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
            <div className="w-full max-w-xl rounded-[1.75rem] bg-white p-6 shadow-2xl dark:bg-slate-900">
                <div className="mb-5 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <h3 className="mt-2 text-xl font-black text-slate-900 dark:text-white">
                            {activeChapter ? `Tambah Latihan untuk ${activeChapter.title}` : 'Tambah Latihan'}
                        </h3>
                    </div>
                ...
                <div className="space-y-4 text-sm text-slate-600 dark:text-slate-300">
                    <p>Modal ini disiapkan untuk input soal latihan per BAB berbentuk pilihan ganda.</p>
                    <p>Field lanjutan yang disarankan: pertanyaan, opsi A-D, dan jawaban benar.</p>
                </div>
